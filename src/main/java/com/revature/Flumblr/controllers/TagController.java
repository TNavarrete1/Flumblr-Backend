...
public class TagController {
    ...
    @GetMapping("/trending")
    public ResponseEntity<List<String>> getTrending(@RequestBody ReportPostRequest postReport,
            @RequestHeader("Authorization") String token) {
        String requesterId = tokenService.extractUserId(token);
        logger.trace("getting trending tags requested by " + requesterId);
        return ResponseEntity.status(HttpStatus.CREATED).body(tagService.getTrending());
    }
}
